...
public class EnemyManagerRana : MonoBehaviour
{
    ...
    public static bool enemyDead = false; // detiene los movimientos  al morir
    ...
    void OnTriggerEnter2D(Collider2D col) //accion al atravesar un collider en formato 2d
    {
        if(col.CompareTag("Bullet"))//da la orden de destruir al enemigo cuando su vida sea igual o menor a cero.
        {
            ...
            {
              enemyDead = true;
              animator.SetBool("exploRana", true); // reproduce la animacion de muerte de BadEye
        ...
        if(col.CompareTag("attack"))
        {
         ...
         if(col.CompareTag("ballAttack"))
         {
            ...
            float bariength = curHealth / enemyHealth;
            SetHealthBar(bariength); // determina el tamaño de la barra cada que  sea atacado
            ...
            {
              enemyDead = true;
              animator.SetBool("exploRana", true); // reproduce la animacion de muerte 
             
              Debug.Log(enemyValue);

              Destroy(gameObject, animatorDelay); // detruye  despues del tiempo concluido de la animacion de muerte
