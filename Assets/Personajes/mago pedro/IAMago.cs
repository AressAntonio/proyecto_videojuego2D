...
public class IAMago : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      {
          PerseguirJugador(); 
          anim.SetFloat("velocidadM", 1);
          anim.SetBool("ataqueM", false);
      }
      else if(Mathf.Abs(distJugador) < 4) //ataca al jugador si se esta -1 de distancia
      {
        anim.SetBool("ataqueM", true);
      }  
      else
      {
          NoPerseguir();
          anim.SetFloat("velocidadM", 0); //no se persigue al jugador y enemigo se queda en su pocicion original
      }
    } 
